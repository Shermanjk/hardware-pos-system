...
export default function StockMovementReport() {
  ...
  const loadReport = useCallback(async () => {
    ...
    try {
      ...
      const res = await axios.get<StockMovementResponse>("/api/reports/stock-movement", {
        headers: authHeaders(),
        params,
      });
  ...
  const columns: Column[] = [
    { key: "date", label: "Date", sortable: true, align: "left", format: fmtDate },
    { key: "product_name", label: "Product", sortable: true, align: "left" },
    { key: "quantity", label: "Quantity", sortable: true, align: "right", format: fmt },
    { key: "movement_type", label: "Movement Type", sortable: true, align: "left" },
    { key: "reason", label: "Reason", sortable: true, align: "left" },
    { key: "performed_by", label: "Performed By", sortable: true, align: "left" },
  ];
  ...
  const handleExport = async (type: "pdf" | "excel" | "csv" | "print") => {
    ...
    try {
      if (type === "csv") {
        exportToCSV(
          data,
          `Stock_Movement_Report_${filters.dateFrom}_to_${filters.dateTo}`,
          columns.map(c => ({ key: c.key as keyof StockMovementData, label: c.label }))
        );
      } else if (type === "print") {
        window.print();
      } else {
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Inventory movement history</p>
        </div>
        {data.length > 0 && (
