...
interface CashReconciliationResponse {
...
export default function CashReconciliationReport() {
  ...
  const [data, setData] = useState<CashReconciliationData[]>([]);
  ...
  const loadReport = useCallback(async () => {
    ...
    try {
      ...
      const res = await axios.get<CashReconciliationResponse>("/api/reports/cash-reconciliation", {
        headers: authHeaders(),
        params,
      });
      ...
      console.error("Failed to load cash reconciliation report:", error);
    } finally {
      setIsLoading(false);
    }
  }, [filters]);
  ...
  const columns: Column[] = [
    ...
    { key: "shift", label: "Shift", sortable: true, align: "left" },
    { key: "expected_cash", label: "Expected Cash", sortable: true, align: "right", format: fmt },
    { key: "actual_cash", label: "Actual Cash", sortable: true, align: "right", format: fmt },
    { key: "variance", label: "Variance", sortable: true, align: "right", format: fmt },
    { key: "status", label: "Status", sortable: true, align: "left" },
    { key: "submitted_at", label: "Submitted At", sortable: true, align: "left", format: fmtDateTime },
  ];
  ...
  const summaryRows: SummaryRow[] = [
    {
      ...
      values: {
        cashier: "",
        shift: "",
        expected_cash: data.reduce((sum, r) => sum + Number(r.expected_cash), 0),
        actual_cash: data.reduce((sum, r) => sum + Number(r.actual_cash), 0),
        variance: data.reduce((sum, r) => sum + Number(r.variance), 0),
        status: "",
        submitted_at: "",
      },
    },
  ...
  const handleExport = async (type: "pdf" | "excel" | "csv" | "print") => {
    ...
    try {
      if (type === "csv") {
        exportToCSV(
          data,
          `Cash_Reconciliation_Report_${filters.dateFrom}_to_${filters.dateTo}`,
          columns.map(c => ({ key: c.key as keyof CashReconciliationData, label: c.label }))
        );
      } else if (type === "print") {
        window.print();
      } else {
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Cash Reconciliation Report</h1>
          <p className="text-sm text-gray-500 mt-0.5">Cash drawer reconciliation and variance tracking</p>
        </div>
        {data.length > 0 && (
      ...
      <ReportTable
        ...
        data={data}
        loading={isLoading}
        emptyMessage="No cash reconciliation data found"
