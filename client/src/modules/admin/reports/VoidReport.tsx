...
export default function VoidReport() {
  ...
  const loadReport = useCallback(async () => {
    ...
    try {
      ...
      const res = await axios.get<VoidResponse>("/api/reports/voids", {
        headers: authHeaders(),
        params,
      });
  ...
  const columns: Column[] = [
    ...
    { key: "void_reason", label: "Void Reason", sortable: true, align: "left" },
  ...
  const handleExport = async (type: "pdf" | "excel" | "csv" | "print") => {
    ...
    try {
      if (type === "csv") {
        exportToCSV(
          data,
          `Void_Report_${filters.dateFrom}_to_${filters.dateTo}`,
          columns.map(c => ({ key: c.key as keyof VoidData, label: c.label }))
        );
      } else if (type === "print") {
        window.print();
      } else {
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Void Report</h1>
          <p className="text-sm text-gray-500 mt-0.5">Voided transactions and approvals</p>
        </div>
        {data.length > 0 && (
      ...
      <ReportTable
        ...
        data={data}
        loading={isLoading}
        emptyMessage="No void data found"
        pageSize={50}
      />
    </div>
  );
