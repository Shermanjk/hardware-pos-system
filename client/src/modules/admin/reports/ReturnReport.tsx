...
export default function ReturnReport() {
  ...
  const loadReport = useCallback(async () => {
    ...
    try {
      ...
      if (filters.resolution !== "all") params.resolution = filters.resolution;
      if (filters.approvedBy) params.approved_by = filters.approvedBy;

      const res = await axios.get<ReturnResponse>("/api/reports/returns", {
        headers: authHeaders(),
        params,
      });
      ...
      console.error("Failed to load return report:", error);
    } finally {
      setIsLoading(false);
    }
  }, [filters]);
  ...
  const columns: Column[] = [
    { key: "return_number", label: "Return #", sortable: true, align: "left" },
    ...
    { key: "resolution", label: "Resolution", sortable: true, align: "left" },
    { key: "amount", label: "Amount", sortable: true, align: "right", format: fmt },
    { key: "cashier", label: "Cashier", sortable: true, align: "left" },
    { key: "approved_by", label: "Approved By", sortable: true, align: "left" },
    { key: "resolved_at", label: "Resolved At", sortable: true, align: "left", format: fmtDateTime },
  ];
  ...
  const summaryRows: SummaryRow[] = [
    {
      ...
      values: {
        ...
        product_name: "",
        resolution: "",
        amount: data.reduce((sum, r) => sum + Number(r.amount), 0),
        cashier: "",
        approved_by: "",
        resolved_at: "",
      },
  ...
  const handleExport = async (type: "pdf" | "excel" | "csv" | "print") => {
    ...
    try {
      if (type === "csv") {
        exportToCSV(
          data,
          `Return_Report_${filters.dateFrom}_to_${filters.dateTo}`,
          columns.map(c => ({ key: c.key as keyof ReturnData, label: c.label }))
        );
      } else if (type === "print") {
        window.print();
      } else {
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Return Report</h1>
          <p className="text-sm text-gray-500 mt-0.5">Product returns and refunds</p>
        </div>
        {data.length > 0 && (
      ...
      <ReportTable
        ...
        data={data}
        loading={isLoading}
        emptyMessage="No return data found"
